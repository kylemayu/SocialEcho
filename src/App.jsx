...
function EndScene({ onPlayAgain, onExit }) {
  return (
    <div style={{ textAlign: "center", padding: 40 }}>
      ...
      <br /><br />
      <button onClick={onExit}>Back to Intro</button>
    </div>
  );
}
...
export default function App() {
 ...
 {scene === SCENES.INTRO && <StartScene onBegin={() => setScene(SCENES.GAMEPLAY)} />}
...
{ scene === SCENES.ENDING && (
        <EndScene
          onPlayAgain={() => {
            setPeople(Array.from({ length: PEOPLE_COUNT }, (_, i) => createPerson(i)));
            setScene(SCENES.GAMEPLAY);
          }}
          onExit={() => setScene(SCENES.INTRO)}
        />
      )}
